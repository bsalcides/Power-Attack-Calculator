function [result1, result2, graphfig] = PAopt(AB, TwoHand, critrange, critmult, critconfirmbonus, multdmg, nonmultdmg, numAtks, BAB, AC, miss, DR, iterations)
...
for i=2:numAtks
    if (length(AB) < numAtks);                AB(i) = AB(end);                              end
    ...
    if (length(DR) < numAtks);                DR(i) = DR(end);                              end
end
...
for i=1:length(PAvalues)
    ...
    parfor j=1:iterations
        ...
        for k=1:numAtks
            ...
            dmgAtks(k) = dmgcalc(actualAB, AC, DR(k), miss, critrange(k), critmult(k), critconfirmbonus(k), actualmultdmg, nonmultdmg(k));
        end
...
set(graphfig,'CloseRequestFcn','set(gcf,"Visible","off");');
set(graphfig, 'Visible', 'off');
set(graphfig, 'ToolBar', 'none');
set(graphfig, 'MenuBar', 'none');
% set(graphfig, 'Icon', 'icon.png');
graph = axes(graphfig);
...
function dmg = dmgcalc(AB, AC, DR, miss, critrange, critmult, critconfirmbonus, multdmg, nonmultdmg)
        ...
        if( ((atkroll >= AC)||(roll == 20)) && (roll ~= 1) ) % hit
            if (roll >= critrange) % critcal threat
                ...
                if (critconfirmroll >= AC) % crit confirmed
                    dmg = max([(multdmg*critmult + nonmultdmg - DR), 0]);
                else % crit not confirmed
                    dmg = max([(multdmg + nonmultdmg - DR), 0]);
                end
            else % regular hit
                 dmg = max([(multdmg + nonmultdmg - DR), 0]);
            end
